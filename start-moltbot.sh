...
// Remove legacy Anthropic provider config from previous runs
if (config.models?.providers?.anthropic) {
    console.log('Removing legacy anthropic provider config');
...
// Base URL override (e.g., for Cloudflare AI Gateway or Moonshot API)
// Usage: Set AI_GATEWAY_BASE_URL or MOONSHOT_BASE_URL to your endpoint like:
//   https://gateway.ai.cloudflare.com/v1/{account_id}/{gateway_id}/openai
//   https://api.moonshot.ai/v1
//   https://api.moonshot.cn/v1
const baseUrl = process.env.AI_GATEWAY_BASE_URL || process.env.MOONSHOT_BASE_URL || '';
const isOpenAI = baseUrl.endsWith('/openai');
const preferOpenAI = isOpenAI && process.env.OPENAI_API_KEY && !process.env.MOONSHOT_API_KEY;
...
if (preferOpenAI) {
    ...
    console.log('Configuring Moonshot provider with base URL:', moonshotBaseUrl);
    config.models = config.models || {};
    config.models.providers = config.models.providers || {};
    ...
    // Include API key in provider config if set (required for Moonshot)
