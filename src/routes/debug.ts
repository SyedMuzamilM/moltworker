...
debug.get('/env', async (c) => {
  return c.json({
    // Kimi Code is the DEFAULT AI provider
    has_kimicode_key: !!c.env.KIMICODE_API_KEY,
    default_ai_provider: c.env.KIMICODE_API_KEY ? 'kimi-code' : (c.env.MOONSHOT_API_KEY ? 'moonshot' : 'none'),
    // Moonshot is optional fallback
